...
<template>
  <div class="tk-textarea">
    ...
    <div class="maxlength" v-if="!!maxlength">
      <voice class="voice" v-model="textAreaVal"></voice>
      温馨提示：
      <span v-if="textAreaVal && !regTest"
        >还能再输入{{ maxlength - textAreaVal.length }}字</span
      >
      <span v-if="!textAreaVal">尚未输入内容</span>
      <span style="color:red;" v-if="textAreaVal && regTest"
        >请不要输入全空字符</span
      >
    </div>
