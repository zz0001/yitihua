<template>
<drag-poper ref="drag" @hasMounted=hasMounted v-if="isShow" :w="800" :h="600" class="codeEditor">
    ...
    <div @click="hasOver" class="button">完成编辑</div>
</drag-poper>
</template>

<script>
import dragPoper from '../drag-poper.vue'
export default {
    name: 'JSONEditor',
    data() {
        return {
    ...
    components: {
        'drag-poper': dragPoper
    },
    methods: {
        hasOver() {
            this.myApp.editorToFormatter();
            this.$nextTick(() => {
    ...
    */
     ...
     */
        ...
        */
         ...
         */
        var JSONFormatter = function (container, options, json) {
            // check availability of JSON parser (not available in IE7 and older)
            if (!window.JSON) {
                throw new Error('您当前使用的浏览器不支持 JSON. \n\n' +
                    '请下载安装最新版本的浏览器, 本站推荐Google Chrome.\n' +
