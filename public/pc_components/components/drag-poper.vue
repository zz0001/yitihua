<template>
<div class="vdr" v-if="isShow" :style="style" :class="active || isActive ? 'active' : 'inactive'" >
    <div @mousedown.stop.prevent="bodyDown($event)" @touchstart.stop.prevent="bodyDown($event)" class="box-header">
    ...
    <div v-for="stick in sticks" class="vdr-stick" :class="['vdr-stick-' + stick, isResizable ? '' : 'not-resizable']" @mousedown.stop.prevent="stickDown(stick, $event)" @touchstart.stop.prevent="stickDown(stick, $event)" :style="vdrStick(stick)">
    </div>
</div>
</template>
...
export default {
    ...
    mounted: function () {
        this.left=this.x||(window.innerWidth-this.w)/2;
        this.top=this.y||(window.innerHeight-this.h)/2
        this.$nextTick(()=>{
