...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    props: {
      remember:{//true是开启滚动位置记忆功能，false为关闭
        type:Boolean,
        default:false
      },
      data: {
...
//      }
    ...
    mounted() {
      ...
      this.$nextTick(() => {
        this.initScroll();
        if(this.remember){
          window[this.$route.path]=window[this.$route.path]||{};
          this.scrollTop=window[this.$route.path].scrollTop||0;
          this.scrollTo(0,this.scrollTop)
          this.param && this.url && this.ajax();
        }
            
        this.$emit('scrollMounted')
      })
    },
    beforeDestroy() {
      this.destroy()
      ...
      _listenScrollEvents() {
        this.finalScrollEvents.forEach((event) => {
          this.scroll.on(camelize(event), (...args) => {
            if(event=='scroll-end'){
              if(this.remember){
                this.scrollTop=args[0].y;
                window[this.$route.path]=window[this.$route.path]||{};
                window[this.$route.path].scrollTop=this.scrollTop;
              }
            }
