<template>
    ...
    <div class="page_content" style="">
        ...
        <!--</div>-->
        <!--</div>-->
        <div id="application_list_html" class="jszx-wrapper">
            <div class="jszx-scroller">
                <div id="application_list_html" class="html_margin">
                </div>
            </div>
        </div>
    </div>
...
<script>
    export default {
        ...
        $("#application_list_page").live("pageshow", function () {
            ...
            $("#application_list_page .right_add").listen("quickClick", function (e) {
                if ($(".max-dialog").is(':visible') == true) {
                	...
                	app.$router.push("../deptHandle/deptHandleEdit.html");
                }
            });
            /*我发起的*/
        ...
        function initDeptWelcome() {
            $("#application_list_html")[0].style.height = window.localInnerHeight - $("#application_list_page .page_header").height() - $("#application_list_page .casewelcome_title").height() + "px";
            initScroll({
                "wrapper": "application_list_html",
                "dir": "y",
                "bounce": false,
                "scrollBar": true,
            ...
            scrollToTop("application_list_html");
            window.checkNum = 1;
            //window.flag1 = false;
            app_deptList(false, window.localStorage.loginId, "XB", "", 1)
            //app_deptList(false, window.localStorage.loginId, "xb", "", 1);
      ...
      //分类
        function depttypeByClass() {
            ...
            html += '               <select style="display: inline-block;width: 72%;"><option>待办</option><option>已办</option></select>';
            html += '            </td>';
                    ...
                    if (!isNull(array) && array.length > 0) {
                        for (var i = 0; i < array.length; i++) {
                           ...
                           /*  html += '       <td style="width:34%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + array[i].checkItem + '</td>'; */
                            ...
                            html += '   </tr>';
                            html += '</table>';
                            var $html = createListRow(html).appendTo("#application_list_html");
                        ...
                        if (parseInt(pageNum) < 2) {
                            noListShow("", "", "", $("#application_list_html"), "");
                        }
                    }
                } else {
                    if (reset) {
                    ...
                    if (!isNull(array) && array.length > 0) {
                        for (var i = 0; i < array.length; i++) {
                           ...
                           /*  html += '       <td style="width:34%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + array[i].checkItem + '</td>'; */
                            ...
                            $html.listen("quickClick", function (e) {
                                var data = $(e.currentTarget).data("data");
                                window.linkage_no = data.linkageno;
                                //window.caseRow_id = data.row_id;
                                //window.caseRow_id = data.caseno;
                                app.$router.push("../deptHandle/deptHandleDetail.html");
...
#application_list_page .page_header_title .jszx-bar-icon-up {
        ...
        .tab_bloack_font p, #application_list_page .jszx-bar-item-2 div p {
            height: 2.8em;
            line-height: 2.8em;
        }
