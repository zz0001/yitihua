<template>
<div data-role="page" id="application_list_page">
    <div class="page_header">
        <img class="left_img back-button" src="/staitc/resources/images/icon_white_back.png">
        <div class="page_header_title">执法申请列表</div>
        <div class="right_img right_btn" style="height:2.5em">筛选</div>
        <div class="right_add right_btn" style="height:2.5em">新增</div>
    </div>
    <div class="page_content" style="">
        <div class="jszx-bar-icon-up jszx-bar-item-2 casewelcome_title gray_border">
            <div class="leader_wait">
                <p>我的交办</p>
            </div>
            <div class="leader_already">
                <p>待办已办任务</p>
            </div>
            <div id="downloadlawApply" style="width:30%;">
                <p>下载模板</p>
            </div>
        </div>
        <!--<div class="jszx-bar-icon-up jszx-bar-item-2 casewelcome_search gray_border">-->
        <!--<div class="case_welcome_bigclass">-->
        <!--<p><span id="case_welcome_bigclass_id">澶у皬绫�</span><img-->
        <!--src="/staitc/resources/images/icon_gray_solid_down.png"/>-->
        <!--</p>-->
        <!--</div>-->
        <!--<div class="case_welcome_time">-->
        <!--<p><span id="case_welcome_time_id">涓婃姤鏃堕棿娈�</span><img-->
        <!--src="/staitc/resources/images/icon_gray_solid_down.png"/></p>-->
        <!--</div>-->
        <!--</div>-->
        <div id="application_wrapper" class="jszx-wrapper">
            <div class="jszx-scroller">
                <div id="application_html" class="html_margin">
                </div>
            </div>
        </div>
    </div>
    <div class="KpiTcbackG" id="applicationblackBg" style="display:none">
    </div>
    <div class="KpiFooterContent2" id="applicationDiv" style="display:none">
        <p class="KpiNameTitle"></p>
        <table class="inp_table" cellpadding="0" cellspacing="0">
            <tr>
                <td style="padding: 1em;" id="downloadlawApplyName">执法申请模板</td>
                <td colspan="2" style="text-align:right;"><span style="color:#3977E6;" id="configapplication">下载</span></td>
            </tr>
            <tr>
                <td style="padding: 1em;"></td>
                <td colspan="2" style="text-align:right;"></td>
            </tr>
        </table>
    </div>
</div>
</template>

<script>
export default {
    mounted() {
        window.searchPage = 2;
        window.applicationQueryType = 1;

        $("#application_list_page").live("pageshow", function () {
            window.applicationWelcomePage = false;
            $("#application_html").empty();
            findZFBZMB();
            if (!window.applicationWelcomePage) {
                if (1 == parseInt(window.applicationQueryType)) {
                    myapplicationtInit();
                } else {
                    alreadyaplicationInit();
                }
                window.applicationWelcomePage = true;
            }

            /*筛选*/
            $("#application_list_page .right_img").listen("quickClick", function (e) {
                if ($(".max-dialog").is(':visible') == true) {
                    $(".max-dialog").remove();
                } else {
                    showSmallChoice("分类", "时间段", function () {
                        setTimeout(function () {
                            depttypeByClass();
                        }, 500);

                    }, function () {
                        setTimeout(function () {
                            caseSearchByTime();
                        }, 500);
                    });
                }
            });
            $("#application_list_page .right_add").listen("quickClick", function (e) {
                if ($(".max-dialog").is(':visible') == true) {
                    $(".max-dialog").remove();
                } else {
                    app.$router.push("../applicationEnforcement/applicationEnforcementEdit.html");
                }
            });
            /*我发起的*/
            $("#application_list_page .leader_wait").listen("quickClick", function (e) {
                /* if(window.flag1){

window.flag1 = false;

$(".max-dialog").remove();
                    window.checkNum = 1;
                    myapplicationtInit();
            	} */
                $(".max-dialog").remove();
                window.checkNum = 1;
                window.applicationQueryType = 1;
                myapplicationtInit();

            });
            /*我处理的*/
            $("#application_list_page .leader_already").listen("quickClick", function (e) {
                /* if(window.flag1){

window.flag1 = false;

$(".max-dialog").remove();
                    window.checkNum = 1;
                    alreadyaplicationInit();
            	} */
                $(".max-dialog").remove();
                window.checkNum = 1;
                window.applicationQueryType = 2;
                alreadyaplicationInit();
            });
            /*返回*/
            $("#application_list_page .left_img").listen("quickClick", function (e) {
                if ($(".max-dialog").is(':visible') == true) {
                    $(".max-dialog").remove();
                } else {
                    app.$router.push("../linkage/linkageList.html");
                }
            });
            //附件下载
            $("#downloadlawApply").listen("quickClick", function (e) {
                $("#applicationblackBg").show();
                $("#applicationDiv").show();
                //$("#applicationDiv").addClass("kdts-botwrap-open");
            });

            $("#configapplication").on("click", function (e) {
                $("#applicationblackBg").hide();
                $("#applicationDiv").hide();
                //  $("#applicationDiv").removeClass("kdts-botwrap-open");
                downloadlawApply();
            });

            $("#applicationblackBg").listen("quickClick", function (e) {
                $("#applicationblackBg").hide();
                $("#applicationDiv").hide();
                //  $("#applicationDiv").removeClass("kdts-botwrap-open");
            });
        });

        function downloadlawApply() {
            var url = window.localStorage.filepath;
            var name = $("#downloadlawApplyName").text();
            console.log(url)
            console.log(name)
            isFileExist(name, window.cacheDirectory, function (result) {
                /*     if(result){
                        jsict.appManager.openLocalFile(function(){}, function(){}, window.cacheDirectoryPath+"/"+name);
                    }else{ */
                showConfirm("<p style='padding:1em;'>确认下载文档吗？</p>", null, null, function () {
                    showLoading("正在下载....");

                    var fileTransfer = new FileTransfer();
                    fileTransfer.download(
                        encodeURI(url),
                        window.cacheDirectoryPath + "/" + name,
                        function (entry) {
                            hideLoading();
                            toast("下载成功！");

                            jsict.appManager.openLocalFile(function () {}, function () {}, window.cacheDirectoryPath + "/" + name);
                        },
                        function (error) {
                            hideLoading();
                            toast("下载失败，请重试");
                        }
                    );
                })
                //   }
            });

        }
        //初始化代办已办
        function initapplicationWelcome() {
            $("#application_wrapper")[0].style.height = window.localInnerHeight - $("#application_list_page .page_header").height() - $("#application_list_page .jszx-bar-icon-up").height() + "px";
            initScroll({
                "wrapper": "application_wrapper",
                "dir": "y",
                "bounce": false,
                "scrollBar": true,
                "pullUpLoading": function (reset) { //上拉
                    if (window.checkNum > window.arrayDataLength) {
                        reset(true);
                        toast("没有更多的数据了");
                    } else {
                        if (window.applicationQueryType == 1) {
                            appEnforcemnt_startUpdeptList(reset, window.localStorage.loginId, "ZF", "", window.searchPage++);
                        } else {
                            app_EnforcementList(reset, window.localStorage.loginId, "ZF", "", window.searchPage++)
                        }

                    }
                },
                "pullDownLoading": function (reset) { //下拉
                    window.checkNum = 1;
                    window.searchPage = 2;
                    if (window.applicationQueryType == 1) {
                        appEnforcemnt_startUpdeptList(reset, window.localStorage.loginId, "ZF", "", 1);
                    } else {
                        app_EnforcementList(reset, window.localStorage.loginId, "ZF", "", 1)
                    }

                }
            });
            scrollToTop("application_wrapper");
            window.checkNum = 1;
            //window.flag1 = false;

            app_EnforcementList(false, window.localStorage.loginId, "ZF", "", 1)
            //app_EnforcementList(false, window.localStorage.loginId, "xb", "", 1);
        }
        //初始化2发起
        function initapplicationWelcome_faqi() {
            $("#application_wrapper")[0].style.height = window.localInnerHeight - $("#application_list_page .page_header").height() - $("#application_list_page .jszx-bar-icon-up").height() + "px";
            initScroll({
                "wrapper": "application_wrapper",
                "dir": "y",
                "bounce": false,
                "scrollBar": true,
                "pullUpLoading": function (reset) { //上拉
                    if (window.checkNum > window.arrayDataLength) {
                        reset(true);
                        toast("没有更多的数据了");
                    } else {
                        if (window.applicationQueryType == 1) {
                            appEnforcemnt_startUpdeptList(reset, window.localStorage.loginId, "ZF", "", window.searchPage++);
                        } else {
                            app_EnforcementList(reset, window.localStorage.loginId, "ZF", "", window.searchPage++)
                        }
                    }
                },
                "pullDownLoading": function (reset) { //下拉
                    window.checkNum = 1;
                    window.searchPage = 2;
                    if (window.applicationQueryType == 1) {
                        appEnforcemnt_startUpdeptList(reset, window.localStorage.loginId, "ZF", "", 1);
                    } else {
                        app_EnforcementList(reset, window.localStorage.loginId, "ZF", "", 1)
                    }
                }
            });
            scrollToTop("application_wrapper");
            window.checkNum = 1;
            appEnforcemnt_startUpdeptList(false, window.localStorage.loginId, "ZF", "", 1);
        }

        //分类
        function depttypeByClass() {
            showSmallTitle(".case_list_bigclass", "4", function () {
                //重置

            }, function () {
                //确定
                window.checkNum = 1;
                var val = $("#enforcement_type option:selected").val();
                app_EnforcementList(false, window.localStorage.loginId, "ZF", val, 1);
            });
            var html = ' <table class="inp_table" cellpadding="0" cellspacing="0">';
            html += '       <tr style="height:10em">';
            html += '           <td style="min-width: 3em;padding:1em;font-size: 20px;" class="gray_line_bottom">请选择类型</td>';
            html += '           <td colspan="2" class="gray_line_bottom" style="text-align: center;">';
            html += '               <select id="enforcement_type" style="display: inline-block;width: 72%;"><option value="1">待办</option><option value="2">已办</option></select>';
            html += '            </td>';

            html += '       </tr>';
            html += '   </table>';
            var $row = createListRow(html).appendTo($("#screen_content1"));
        }
        //时间段筛选
        function caseSearchByTime() {
            titleScreenSearchHelp("price", function () {
                //                    cleanScrollSearchList();
            }, function () {
                showSmallTitle(".case_welcome_time", 1, function () {
                    //重置
                    $("#screen_content1 div").removeClass("red_font");
                    window.caseStartTime = "";
                    window.caseEndTime = "";
                    $("#dept_start_timer").val("");
                    $("#dept_over_timer").val("");
                }, function () {
                    //确定
                    window.searchPage = 2;
                    window.caseStartTime = $("#dept_start_timer").val();
                    window.caseEndTime = $("#dept_over_timer").val();
                    app_EnforcementList(false, window.localStorage.loginId, "ZF", "", 1);
                });
                var html = ' <table class="inp_table" cellpadding="0" cellspacing="0">';
                html += '       <tr>';
                html += '           <td style="min-width: 5em;padding:1em;" class="gray_line_bottom">开始时间</td>';
                html += '           <td class="gray_line_bottom" style="text-align: center;">';
                html += '               <input id="dept_start_timer" style="text-align: center;" placeholder="请选择开始时间" readonly  type="text"/>';
                html += '            </td>';
                html += '           <td class="gray_line_bottom" style="width:2em;min-width:2em;max-width:2em;padding:0em 1em 0em 0em;">';
                html += '               <img class="div_input_value_img" src="/staitc/resources/images/icon_gray_solid_down.png">';
                html += '           </td>';
                html += '       </tr>';
                html += '       <tr>';
                html += '           <td style="min-width: 5em;padding: 1em;" class="gray_line_bottom">结束时间</td>';
                html += '           <td class="gray_line_bottom" style="text-align: center;">';
                html += '               <input id="dept_over_timer" style="text-align: center" readonly placeholder="请选择结束时间" style="padding:0em 1.5em 0em 0em;"  type="text"/>';
                html += '           </td>';
                html += '           <td class="gray_line_bottom" style="width:2em;min-width:2em;max-width:2em;padding:0em 1em 0em 0em;text-align: right;">';
                html += '               <img class="div_input_value_img" src="/staitc/resources/images/icon_gray_solid_down.png">';
                html += '           </td>';
                html += '       </tr>';
                html += '   </table>';
                var $row = createListRow(html).appendTo($("#screen_content1"));
                chooseDate("dept_start_timer", "date");
                chooseDate("dept_over_timer", "date");
                $(".alert_edit div").css("margin", "0em");
                $(".timer_btn").css("margin-top", "2em");
                $(".timer_btn").css("margin-left", "13%");
                if (!isNull(window.caseStartTime)) {
                    $("#dept_start_timer").val(window.caseStartTime);
                }
                if (!isNull(window.caseEndTime)) {
                    $("#dept_over_timer").val(window.caseEndTime);
                }
                $row.find(".timer_btn").listen("quickClick", function (e) {
                    //鐐瑰嚮浜嬩欢
                    var data = $(e.currentTarget).data("data");
                });
            });
        }
        //清空缓存
        function cleanapplicationWelcomeList() {
            window.searchPage = 2;
            window.noticeTypeName = "";
            window.caseStartTime = "";
            window.caseEndTime = "";
            $("#application_list_page #case_welcome_inp").val("");
            window.choiceThingsParts = "";
            window.choiceBigType = "";
            window.choiceSmallType = "";

            setRestTitle();
        }

        //wode
        function myapplicationtInit() {
            cleanapplicationWelcomeList();
            //鏈姙
            $("#application_list_page .casewelcome_title div").removeClass("tab_oranger_font");
            $("#application_list_page .leader_wait").addClass("tab_oranger_font");
            $("#application_list_page .casewelcome_search").hide();
            $("#application_list_page .right_img").hide();
            $("#application_list_page .right_add").show();
            //            $("#application_list_page #case_welcome_rednum").hide();
            window.leaderQueryType = "1";
            //initapplicationWelcome();
            initapplicationWelcome_faqi();
        }

        //aleady
        function alreadyaplicationInit() {
            cleanapplicationWelcomeList();
            $("#application_list_page .right_img").show();
            $("#application_list_page .right_add").hide();
            $("#application_list_page .casewelcome_title div").removeClass("tab_oranger_font");
            $("#application_list_page .leader_already").addClass("tab_oranger_font");
            //宸插姙
            //            appWorkOrder_todoTotalByType();
            $("#application_list_page .casewelcome_search").show();
            //            $("#application_list_page #case_welcome_rednum").show();
            window.leaderQueryType = "2";
            initapplicationWelcome();
        }

        /**
         * 筛选条件
         * @param type
         */
        function setRestTitle() {
            //$("#case_welcome_bigclass_id").html("大小类");
            $("#case_welcome_time_id").html("上报时间段");
        }
        /**
         *列表
         * @param queryType 查询类型  1：待办 2：已办
         * @param startTime 开始时间
         * @param endTime 结束时间
         * @param eventComponent 事件部门
         * @param bigClass 大类
         * @param smallClass 小类
         */
        function app_EnforcementList(reset, loginId, flag, status, pageNum) {
            if (!reset) {
                //showLoading("正在搜索...");
            }
            if (parseInt(pageNum) < 2) {
                $("#application_html").empty();
            }
            var obj = "";
            obj += toJson("loginId", window.localStorage.loginId) + "&";
            obj += toJson("flag", flag) + "&";
            obj += toJson("status", status) + "&";
            obj += toJson("pageSize", "20") + "&";
            obj += toJson("pageNum", pageNum);

            doPost("caseDepartmentList.action", obj, function (data) {
                if (!reset) {
                    hideLoading();
                }
                if (data.code == 0) {
                    if (reset) {
                        reset(true);
                    }
                    var array = data.datas;
                    getDataLength(data.totalSize, 6);
                    if (data.datas.length < 20) {
                        console.log('进来了')
                        window.checkNum = 2;
                    }
                    console.log("长度：" + data.totalSize);
                    if (!isNull(array) && array.length > 0) {
                        for (var i = 0; i < array.length; i++) {

                            var html = '';
                            html += '<table cellspacing="0" class="list_second_table gray_line_bottom" cellpadding="0">';

                            html += '   <tr>';
                            html += '       <td style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"> ' + array[i].linkagename + '</td>';
                            /*  html += '       <td style="width:34%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + array[i].checkItem + '</td>'; */
                            html += '       <td style="padding-left:0.5em;width: 5em;" class="oranger_font">' + array[i].status + '</td>';
                            html += '   </tr>';
                            html += '   <tr>';
                            html += '       <td colspan="3" style="text-align:left;"> ' + array[i].deadLine + '</td>';
                            html += '   </tr>';
                            html += '</table>';
                            var $html = createListRow(html).appendTo("#application_html");
                            $html.data('data', array[i]);
                            $html.listen("quickClick", function (e) {
                                var data = $(e.currentTarget).data("data");
                                window.row_id = data.row_id;
                                window.currjump = "list";
                                //window.caseRow_id = data.row_id;
                                //window.caseRow_id = data.caseno;
                                app.$router.push("../applicationEnforcement/applicationEnforcementDetail.html");

                            });
                        }
                    } else {
                        if (parseInt(pageNum) < 2) {
                            noListShow("", "", "", $("#application_html"), "");
                        }
                    }
                } else {
                    if (reset) {
                        reset(true);
                    }
                    toast(data.message);
                }
                // window.flag1 = true;
            }, "");
        }
        //我的发起
        function appEnforcemnt_startUpdeptList(reset, loginId, flag, status, pageNum) {
            if (!reset) {
                hideLoading();
            }
            if (parseInt(pageNum) < 2) {
                $("#application_html").empty();
            }
            var obj = "";
            obj += toJson("loginId", window.localStorage.loginId) + "&";
            obj += toJson("flag", flag) + "&";
            obj += toJson("status", "") + "&";
            obj += toJson("pageSize", "20") + "&";
            obj += toJson("pageNum", pageNum);

            doPost("myCaseReportList.action", obj, function (data) {
                if (!reset) {
                    hideLoading();
                }

                if (data.code == 0) {
                    if (reset) {
                        reset(true);
                    }
                    var array = data.datas;
                    getDataLength(data.totalSize, 6);
                    if (data.datas.length < 20) {
                        console.log('进来了')
                        window.checkNum = 2;
                    }
                    console.log("长度：" + data.totalSize);
                    if (!isNull(array) && array.length > 0) {
                        for (var i = 0; i < array.length; i++) {

                            var html = '';
                            html += '<table cellspacing="0" class="list_second_table gray_line_bottom" cellpadding="0">';

                            html += '   <tr>';
                            html += '       <td style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"> ' + array[i].linkagename + '</td>';
                            /*  html += '       <td style="width:34%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + array[i].checkItem + '</td>'; */
                            html += '       <td style="padding-left:0.5em;width: 5em;" class="oranger_font">' + array[i].status + '</td>';
                            html += '   </tr>';
                            html += '   <tr>';
                            html += '       <td colspan="3" style="text-align:left;"> ' + array[i].deadLine + '</td>';
                            html += '   </tr>';
                            html += '</table>';
                            var $html = createListRow(html).appendTo("#application_html");
                            $html.data('data', array[i]);
                            $html.listen("quickClick", function (e) {
                                var data = $(e.currentTarget).data("data");
                                window.row_id = data.row_id;
                                window.currjump = "list";
                                //window.caseRow_id = data.row_id;
                                //window.caseRow_id = data.caseno;
                                app.$router.push("../applicationEnforcement/applicationEnforcementDetail.html");

                            });
                        }
                    } else {
                        if (reset) {
                            reset(true);
                        }
                        if (parseInt(pageNum) < 2) {
                            noListShow("", "", "", $("#application_html"), "");
                        }
                    }
                } else {
                    if (reset) {
                        reset(true);
                    }
                    toast(data.message);
                    noListShow("", "", "", $("#application_html"), "");
                }
            }, "");
        }

        //下载
        function findZFBZMB() {
            var obj = "";
            doPost("findZFBZMB.action", obj, function (data) {
                console.log(data.filepath)
                window.localStorage.filepath = window.imgUrl + data.filepath;
            }, "");
        }
    }
}
</script>

<style lang="less" scoped>
#application_list_page .page_header_title .jszx-bar-icon-up {
    background: #fafafa;
}

#application_list_page .list_table tr:first-child td {
    padding: 1em 0em 0em 1.3em;
}

#application_list_page .list_table tr td {
    padding: 0.7em 0em 1em 1.3em;
}

#application_list_page .home_list_tr_first_td {
    padding: 0.7em;
    padding-left: 0em;
}

/*#application_list_page .page_header .right_img {*/
/*right: 1.5em;*/
/*}*/

#application_list_page .red_num {
    /*margin-left: 3.3em;*/
    float: right;
    top: 1.2em;
    right: 2em;
}

#application_list_page .search_div {
    padding: 0.5em 1em 0em 1em;
    margin: 0em;
    background: #edeef1;
}

#application_list_page #case_welcome_inp {
    width: 65%;
    float: left;
    line-height: 2.5em;
    height: 2.5em;
}

#application_list_page #refresh_service {
    width: 2em;
    height: 2em;
    padding: 0.4em 0.2em 0em 0.8em;
    float: right;
}

#application_list_page #service_search {
    width: 4em;
    text-align: center;
    color: #ffffff;
    margin-top: 0.7em;
    float: right;
    line-height: 2.2em;
    border-radius: 0.3em;
}

#application_list_page .search_div {
    height: 3.2em;
}

#application_list_page .home_list_tr_first_td span {
    margin-left: 0.5em;
}

/*#application_list_page table tr:first-child td:first-child {*/
/*padding: 0.7em 0em 0.7em 0.5em;*/
/*line-height: 1.3em;*/
/*text-align: left;*/
/*}*/

/*#application_list_page table tr:last-child td {*/
/*padding:0em 0em 0.6em 0.5em;*/
/*line-height: 1.3em;*/
/*}*/

/*#application_list_page table tr:first-child td:last-child {*/
/*text-align: right;*/
/*vertical-align: middle;*/
/*max-width: 6em;*/
/*min-width: 6em;*/
/*}*/

/*#application_list_page table:last-child tr td:last-child {*/
/*padding: 0.3em 1em 0.7em 0.6em;*/
/*text-align: left;*/
/*}*/
#application_list_page .small_img {
    height: 1.2em;
}

#application_list_page .jszx-bar-item-2>div {
    width: 50%;
}

#application_list_page .tab_oranger_font p,
.tab_bloack_font p,
#application_list_page .jszx-bar-item-2 div p {
    height: 2.8em;
    line-height: 2.8em;
}

/*绛涢�夊皬鍥炬爣*/
#application_list_page .jszx-bar-item-2 div p img {
    width: 0.6em;
    height: 0.5em;
    padding-left: 0.5em;
}

#application_list_page .right_btn {
    width: 4.5em;
    right: 0em;
    height: 2.5em;
}

#application_list_page .jszx-bar-item-2>div {
    width: 35%;
}

#application_list_page .jszx-bar-item-2 p,
.jszx-bar-item-2 p {
    height: 2.8em;
    line-height: 2.8em;
}

/*复选框样式*/
.KpiTcbackG {
    position: absolute;
    top: 0;
    left: 0;
    bottom: 0;
    right: 0;
    z-index: 20;
    background: rgba(0, 0, 0, 0.5);
}

.KpiTcbackG .Tipscontent {
    width: 81%;
    margin: 40% auto 0;
    background: #fff;
    border-radius: 6px;
    padding: 5%;
    height: 350px;
    overflow-y: auto;
}

.webkit-appearance {
    -webkit-appearance: checkbox !important;
}

.disI {
    display: inline-block;
}

.vm {
    vertical-align: middle;
}

.f14 {
    font-size: 14px;
}

.KpiFormBut {
    width: 30%;
    background: #f5a035;
    padding: 1.5%;
    border-radius: 3px;
    color: #fff;
    display: inline-block;
}

.tc {
    text-align: center;
}

.mt5 {
    margin-top: 5%;
}

/*复选框样式end*/
.KpiFooterContent2 {
    background: #fff;
    position: absolute;
    bottom: 0;
    left: 0;
    right: 0;
    z-index: 25;
    padding: 0 5% 5% 5%;

}
</style>
