<template>
    <div data-role="page" id="zfxc_edit_page">
        ...
        <div class="page_content">
            <div id="zfxc_edit_wrapper" class="jszx-wrapper">
                        ...
                        <table class="inp_table" cellpadding="0" cellspacing="0">
                            ...
                            <tr>
                                <td colspan="3" class="commodityAddImgs">
                                    <div id="zfxc_edit_img_wrapper" class="jszx-wrapper">
                                        <div class="jszx-scroller" id="zfxc_edit_img_scroll">
                                            <div id="zfxc_edit_img"></div>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr id="zfxc_location_tr">
                                <td style="min-width: 5em;" class="">位置描述<span>*</span></td>
                                <td style="padding:1em 0.5em 1em 0em;line-height: 1.4em;text-align: right;"
                                ><div contenteditable="true" id="zfxc_edit_address"></div></td>
                                <td style="width: 1%;" id="zfxc_edit_address_img">
                            ...
                            <tr>
                                <td colspan="3">
                            <textarea
                                    onfocus="newCheckWord('zfxc_edit_problem',500,'zfxc_case_words')"
                                    onKeyUp="javascript:newCheckWord('zfxc_edit_problem',500,'zfxc_case_words');"
                                    onMouseDown="javascript:newCheckWord('zfxc_edit_problem',500,'zfxc_case_words');"
                                    placeholder="请输入问题描述" id="zfxc_edit_problem"
        ...
        <div class="page_footer">
            <div class="footer_button_div">
                <div id="zfxc_save_returnAddress_leader" class="alert_bottom_three orange_background">保存</div>
            </div>
        </div>
    ...
    export default {
        ...
        mounted(){
            ...
            window.searchPage = 2;
                ...
                initsspCaseEdit();
                /*返回*/
                $("#zfxc_edit_page .back-button").listen("quickClick", function (e) {
                    // $.mobile.changePage("../load/welcome.html");
                    that.$router.push({path:'welcome',query:{}});
                });
                //位置
                $("#zfxc_edit_page #zfxc_edit_address_img").listen("quickClick", function (e) {
                    showMapAlert("请选择位置", "确定", "", function () {
                        $("#zfxc_edit_address").html($("#map_html_address").html());
                ...
                //商品图片
                $("#zfxc_edit_page #choicemore").click(function (e) {
                    window.upload_imgscroll = "zfxc_edit_img_scroll";
                    window.upload_imgwrapper = "zfxc_edit_img_wrapper";
                    window.upload_addimg = "zfxc_edit_img";
                    getPhotograph();
                });
                $("#zfxc_edit_page .right_img").listen("quickClick", function (e) {
                    window.originType="5";
                    // $.mobile.changePage("../hsxcy/ycjList.html");
                    that.$router.push({path:'ycjList',query:{}});
                });
                //截止时间
                chooseDate("ssp_edit_endTime", "datetime");


                //处置部门
                $("#zfxc_edit_page #case_doDept").listen("quickClick", function (e) {
                    app_getDeptType();
                });
                /*保存*/
                $("#zfxc_edit_page .page_footer").listen("quickClick", function (e) {
                    ...
                    var flag=checkTextSpace($("#zfxc_edit_problem").val());
                    if(window.leadercommitFlag){

                    }else{
                        if (isNull($("#zfxc_edit_problem").val())||flag==false) {
                            if(flag==false){
                                toast("问题描述请不要输入空格");
                            }else{
                                toast("请输入问题描述");
                            ...
                            showContentAlert("确认保存本次提交吗？", function () {
                                ...
                                $("#zfxc_save_returnAddress_leader").removeClass("orange_background");
                                $("#zfxc_save_returnAddress_leader").addClass("gray_gray_background");
                                if (!isNull(window.picture_array) || window.picture_array.length > 0) {
                                    ...
                                    appWorkOrder_leaderOrderReport("", $("#zfxc_edit_problem").val());
                                }
                            }, "确定");
                        }
                    }
                ...
                doPost("app_publicOpinion_upPic.action", obj, function (data) {
                    if (data.code == 0) {
                        ...
                        if (parseInt(type) == window.picture_array.length) {
                            appWorkOrder_leaderOrderReport("", $("#zfxc_edit_problem").val(), $("#ssp_edit_endTime").val(), getNowTime(), window.doDeptArr);
            ...
            function initsspCaseEdit() {
                $("#zfxc_edit_wrapper")[0].style.height = window.localInnerHeight - $("#zfxc_edit_page .page_header").height() - $("#zfxc_edit_page .page_footer").height() + "px";
                initScroll({
                    "wrapper": "zfxc_edit_wrapper",
                    "dir": "y",
                    "bounce": false,
                    "scrollBar": true
                });
                scrollToTop("zfxc_edit_wrapper");
                leadersspCaseEdit();
                window.leadercommitFlag=0;
            }
            function leadersspCaseEdit() {
                $("#zfxc_edit_page table tr td").addClass("gray_line_bottom");
                $("#zfxc_edit_page table tr td input").val("");
                $("#zfxc_edit_page table tr td textarea").val("");
                //图片集合
                ...
                initMapInfo("#zfxc_edit_address");
                window.choiceThingsParts = "";// 职能事项：
                window.choiceBigType = "";//部件大类：
                window.choiceSmallType = "";//部件小类
                window.choiceOneGridId = "";//网格
                $("#zfxc_edit_img_scroll").css("transform", "none");
                $("#zfxc_edit_img_wrapper .scroll-bar-x").css("transform", "none");
                $("#ssp_words").val("0/150");
                $("#zfxc_edit_page #zfxc_edit_img").empty();
                $("#zfxc_edit_page #zfxc_edit_img").append('<img class="goodsimgone" style="padding-top:2em;" src="/staitc/resources/images/icon_add_photo.png" id="choicemore">');
            }
            //部门名称
            function app_getDeptType(type, source) {
                ...
                $('#zfxc_edit_page').append($(info));
                //$('#ssp_one_html').html(info);
                ...
                doPost("chooseDepartment.action", obj, function (data) {
                    if (!isNull(data) && data.length > 0) {
                        ...
                        for (var i = 0; i < array.length; i++) {
                            ...
                            $('#zfxc_edit_page').find('.checkboxList').append(info1);
                            $('#zfxc_edit_page').find('.checkboxList2').append(info2);
                            $('#zfxc_edit_page').find('.checkboxList3').append(info3);
                        }
                        //确定按钮
                        $('#zfxc_edit_page').find('.KpiFormBut').listen("quickClick", function (e){
                            var str = '';
                            if(window.doDeptArr.length>0){
                                for(var i=0 ; i < window.doDeptArr.length; i++){
                            ...
                            $('#zfxc_edit_page').find('.KpiTcbackG').hide();
                            $("#case_doDept").val(str);

                        });
                        $('#zfxc_edit_page').find('.KpiTcbackG').show();
            ...
            function appWorkOrder_leaderOrderReport(componentIdNo, problem, deadline, reportTime, doDeptArr) {
                ...
                var address=$("#zfxc_edit_address").text();
                var obj = "";
                obj += toJson("origin", "5") + "&";//工单来源 2：直派，3：随手拍
                ...
                doPost("dsb.action", obj, function (data) {
                    window.leadercommitFlag=0;
                    $("#zfxc_save_returnAddress_leader").removeClass("gray_gray_background");
                    $("#zfxc_save_returnAddress_leader").addClass("orange_background");
                    if (data.code == 0) {
                        hideLoading();
                        toast(data.message);
                        cleadCaseEdit();
...
<style scoped>
    #zfxc_edit_page .page_header .right_img {
        width: 4.5em;
        right: 0em;
        height: 2.5em;
    }
    ...
    /*img start*/
    #zfxc_edit_page .commodityAddImgs {
        overflow: hidden;
        background: #f3f3f3;
    }

    #zfxc_edit_page .imgdiv {
        padding: 0.7em;
        padding-top: 0em;
        position: relative;
        width: 6em;
    ...
    /*img end*/
    #zfxc_edit_page .div_input_value_img {
        width: 1.5em;
        height: 1.5em;
        padding-right: 0.5em;
    }
