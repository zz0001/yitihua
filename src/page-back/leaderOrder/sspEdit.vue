<template>
    <div data-role="page" id="ssp_edit_page">
        ...
        <div class="page_content">
            ...
            <div id="ssp_edit_wrapper" class="jszx-wrapper">
                    ...
                    <div class="html_margin white_background">
                        ...
                        <table class="inp_table" cellpadding="0" cellspacing="0">
                            ...
                            <tr>
                                <td colspan="3">
                            <textarea
                                    onfocus="newCheckWord('ssp_edit_problem',500,'case_words')"
                                    onKeyUp="javascript:newCheckWord('ssp_edit_problem',500,'case_words');"
                                    onMouseDown="javascript:newCheckWord('ssp_edit_problem',500,'case_words');"
                                    placeholder="请输入问题描述" id="ssp_edit_problem"
...
<script>
    export default {
        ...
        mounted(){
            var that=this;
            window.searchPage = 2;
            // $("#ssp_edit_page").live("pageshow", function () {
                initsspCaseEdit();
                /*返回*/
                $("#ssp_edit_page .back-button").listen("quickClick", function (e) {
                    //$.mobile.changePage("../load/welcome.html");
                    that.$router.push({path:'welcome',query:{}});
                });
                //位置
                $("#ssp_edit_page #ssp_edit_address_img").listen("quickClick", function (e) {
                    showMapAlert("请选择位置", "确定", "", function () {
                        $("#ssp_edit_address").html($("#map_html_address").html());
                ...
                //商品图片
                $("#ssp_edit_page #choicemore").click(function (e) {
                    window.upload_imgscroll = "ssp_edit_img_scroll";
                    window.upload_imgwrapper = "ssp_edit_img_wrapper";
                    window.upload_addimg = "ssp_edit_img";
                    getPhotograph();
                });
                $("#ssp_edit_page .right_img").listen("quickClick", function (e) {
                    window.originType="3";
                ...
                //处置部门
                $("#ssp_edit_page #case_doDept").listen("quickClick", function (e) {
                    app_getDeptType();
                });
                /*保存*/
                $("#ssp_edit_page .page_footer").listen("quickClick", function (e) {
                    ...
                    // console.log(window.picture_array)
                    // publicOpinion_upPicture(window.picture_array[0],1);
                    var flag=checkTextSpace($("#ssp_edit_problem").val());
                    if(window.leadercommitFlag){

                    }else{
                        if (isNull($("#ssp_edit_problem").val())||flag==false) {
                            if(flag==false){
                                toast("问题描述请不要输入空格");
                            }else{
                                toast("请输入问题描述");
                                ...
                                $events:{
                                    confirm(A){
                                        ...
                                        if (!isNull(window.picture_array) || window.picture_array.length > 0) {
                                            ...
                                            appWorkOrder_leaderOrderReport("", $("#ssp_edit_problem").val());
                                        }
                                    }
                                }
                            }).show()
                ...
                doPost("app_publicOpinion_upPic.action", obj, function (data) {
                    if (data.code == 0) {
                        ...
                        if (parseInt(type) == window.picture_array.length) {
                            appWorkOrder_leaderOrderReport("", $("#ssp_edit_problem").val(), $("#ssp_edit_endTime").val(), getNowTime(), window.doDeptArr);
            ...
            function initsspCaseEdit() {
                $("#ssp_edit_wrapper")[0].style.height = window.localInnerHeight - $("#ssp_edit_page .page_header").height() - $("#ssp_edit_page .page_footer").height() + "px";
                initScroll({
                    "wrapper": "ssp_edit_wrapper",
                    "dir": "y",
                    "bounce": false,
                    "scrollBar": true
                });
                scrollToTop("ssp_edit_wrapper");
                leadersspCaseEdit();
                window.leadercommitFlag=0;
                //locate();
            }
            function leadersspCaseEdit() {
                $("#ssp_edit_page table tr td").addClass("gray_line_bottom");
                $("#ssp_edit_page table tr td input").val("");
                $("#ssp_edit_page table tr td textarea").val("");
                //图片集合
                ...
                initMapInfo("#ssp_edit_address");
                window.choiceThingsParts = "";// 职能事项：
                window.choiceBigType = "";//部件大类：
                window.choiceSmallType = "";//部件小类
                window.choiceOneGridId = "";//网格
                $("#ssp_edit_img_scroll").css("transform", "none");
                $("#ssp_edit_img_wrapper .scroll-bar-x").css("transform", "none");
                $("#ssp_words").val("0/150");
                $("#ssp_edit_page #ssp_edit_img").empty();
                $("#ssp_edit_page #ssp_edit_img").append('<img class="goodsimgone" style="padding-top:2em;" src="/staitc/resources/images/icon_add_photo.png" id="choicemore">');
            }
            //部门名称
                ...
                doPost("chooseDepartment.action", obj, function (data) {
                    if (!isNull(data) && data.length > 0) {
                        ...
                        for (var i = 0; i < array.length; i++) {
                            ...
                            $('#ssp_edit_page').find('.checkboxList').append(info1);
                            $('#ssp_edit_page').find('.checkboxList2').append(info2);
                            $('#ssp_edit_page').find('.checkboxList3').append(info3);
                        }
                        //确定按钮
                        $('#ssp_edit_page').find('.KpiFormBut').listen("quickClick", function (e){
                            var str = '';
                            if(window.doDeptArr.length>0){
                                for(var i=0 ; i < window.doDeptArr.length; i++){
                            ...
                            $('#ssp_edit_page').find('.KpiTcbackG').hide();
                            $("#case_doDept").val(str);

                        });
                        $('#ssp_edit_page').find('.KpiTcbackG').show();
            ...
            function appWorkOrder_leaderOrderReport(componentIdNo, problem, deadline, reportTime, doDeptArr) {
                ...
                var address=$("#ssp_edit_address").text();
                var obj = "";
