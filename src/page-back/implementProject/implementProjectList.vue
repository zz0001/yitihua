<template>
    <div data-role="page" id="implement_list_page">
        ...
        <div class="page_content" style="">
            ...
            <div id="implement_wrapper" class="jszx-wrapper">
                <div class="jszx-scroller">
                    <div id="implement_html" class="html_margin white_background">
    ...
    export default {
        ...
        mounted(){
            ...
            window.searchPage = 2;
                ...
                /*上报*/
                $("#implement_list_page .right_img").listen("quickClick", function (e) {

                });
                /*返回*/
                $("#implement_list_page .left_img").listen("quickClick", function (e) {
                ...
                //筛选
                $("#implement_list_page .right_btn").listen("quickClick", function (e) {
            ...
            function initImplementWelcome() {
                $("#implement_wrapper")[0].style.height = window.localInnerHeight - $("#implement_list_page .page_header").height() - $("#implement_list_page .jszx-bar-icon-up").height() + "px";
                initScroll({
                    "wrapper": "implement_wrapper",
                    "dir": "y",
                    "bounce": false,
                    "scrollBar": true,
                    "pullUpLoading": function (reset) { //上拉
                        if (window.searchPage > window.arrayDataLength) {
                            ...
                            if(window.projecrtype=="-1"){
                                app_findUnStartedList(reset, window.searchPage++);
                            }else if(window.projecrtype=="-2"){

                                app_findStartedList(reset, window.searchPage++);
                            }else if(window.projecrtype=="-3"){

                                app_findFinishList(reset, window.searchPage++);
                            }else if(window.projecrtype=="1"||window.projecrtype=="2"||window.projecrtype=="3"||window.projecrtype=="4"||window.projecrtype=="5"||window.projecrtype=="6"||window.projecrtype=="7"){
                                app_findTotalList(reset, window.searchPage++, window.projecrtype)
                            }else{
                    ...
                    "pullDownLoading": function (reset) { //下拉
                        ...
                        if(window.projecrtype=="-1"){
                            app_findUnStartedList(reset, 1);
                        }else if(window.projecrtype=="-2"){

                            app_findStartedList(reset, 1);
                        }else if(window.projecrtype=="-3"){

                            app_findFinishList(reset, 1);
                        }else if(window.projecrtype=="1"||window.projecrtype=="2"||window.projecrtype=="3"||window.projecrtype=="4"||window.projecrtype=="5"||window.projecrtype=="6"||window.projecrtype=="7"){
                            app_findTotalList(reset, 1, window.projecrtype)
                        }else{
                            app_implementProjectList(reset, 1)
                        }
                    }
                });
                scrollToTop("implement_wrapper");

                //开始时间

                //结束时间
                if(window.projecrtype=="-1"){
                    app_findUnStartedList(false, 1);
                }else if(window.projecrtype=="-2"){

                    app_findStartedList(false, 1);
                }else if(window.projecrtype=="-3"){

                    app_findFinishList(false, 1);
                }else if(window.projecrtype=="1"||window.projecrtype=="2"||window.projecrtype=="3"||window.projecrtype=="4"||window.projecrtype=="5"||window.projecrtype=="6"||window.projecrtype=="7"){
                    app_findTotalList(false, 1, window.projecrtype)
                }else{
                    app_implementProjectList(false, 1);
                }
                    ...
                    if (data.total > 0) {
                        ...
                        if (!isNull(array) && array.length > 0) {
                            for (var i = 0; i < array.length; i++) {
                                ...
                                var $html = createListRow(html).appendTo("#implement_html");
            ...
            //未开工
            //接口调用以及dom构建
            function app_findUnStartedList(reset, pageNum) {
                if (!reset) {
                    //showLoading("正在搜索...");
                }
                if (parseInt(pageNum) < 2) {
            ...
            //已开工
            function app_findStartedList(reset, pageNum) {
                if (!reset) {
                    //showLoading("正在搜索...");
                }
                if (parseInt(pageNum) < 2) {
            ...
            //已竣工
            function app_findFinishList(reset, pageNum) {
                if (!reset) {
                    //showLoading("正在搜索...");
                }
                if (parseInt(pageNum) < 2) {
                    ...
                    if (data.total > 0) {
                        ...
                        if (!isNull(array) && array.length > 0) {
                            for (var i = 0; i < array.length; i++) {
                                ...
                                html += '   </tr>';
                                html += '   <tr >';
                                html += '       <td colspan="3" class="list_top_down"><div style="display: inline-block;width:21%;text-align:center;"><div>总投资</div><div class="oranger_font">' + totalinvestment + '</div></div> ';
                                html +='<div style="background:#dadada;width: 1px;height:2em;display: inline-block;top: 50%; margin-top: -0.5em;"></div>'
                                html += '<div style="display: inline-block;width:25%;text-align:center;"><div>年度投资</div><div class="oranger_font">' + investmentamount + '</div></div>';
                                html +='<div style="background:#dadada;width: 1px;height:2em;display: inline-block;top: 50%; margin-top: -0.5em;"></div>'
                                html += '<div style="display: inline-block;width:25%;text-align:center;"><div>月投资</div><div class="oranger_font">' + yearcompleteamount + '</div></div>';
                                html +='<div style="background:#dadada;width: 1px;height:2em;display: inline-block;top: 50%; margin-top: -0.5em;"></div>'
                                html += '<div style="display: inline-block;width:25%;text-align:center;"><div>年度完成率</div><div class="oranger_font">' + yearcompleterate + '</div></div>';

                                html += '</td> </tr>';
                                html += '   <tr style="background: #f5f5f5;">';
                                html += '       <td colspan="3" style="border-top:1px solid #dadada;font-size:14px;text-align:left;padding: 0.5em 0;padding-bottom: 0;"><div style="display: inline-block;width:33%;text-align:center;"><div>检查督办</div><div class="oranger_font">' + array[i].superviseno + '</div></div> ';
                                html +='<div style="background:#dadada;width: 1px;height:2em;display: inline-block;top: 50%; margin-top: -0.5em;"></div>'

                                html += '<div style="display: inline-block;width:32%;text-align:center;"><div>存在问题</div><div class="green_font" style="color:#3d8607;">' + array[i].issueno + '</div></div>';
                                var responsibleunits=array[i].responsibleunits;
                                if(array[i].responsibleunits==""){
                                    responsibleunits="无";
                                }
                                ...
                                html += '<div style="display: inline-block;width:33%;text-align:center;"><div>责任单位</div><div class="common_font" style="color:#333333;">' + responsibleunits + '</div></div>';
...
<style scoped>
    #implement_list_page .page_header_title .jszx-bar-icon-up {
        background: #fafafa;
    }

    #implement_list_page .home_list_tr_first_td {
        padding: 0.7em;
        padding-left: 0em;
    }

    #implement_list_page .right_btn {
        width: 3.5em;
        height: 1.5em;
        color: #ffffff;
    }

    #implement_list_page .right_btn .btn_div {
        color: #ffffff;
    ...
    #implement_list_page .jszx-bar-item-3 > div {
        width: 33.3%;
    }

    #implement_list_page .tab_oranger_font p,
    .tab_bloack_font p, #implement_list_page .jszx-bar-item-3 div p {
        height: 2.8em;
        line-height: 2.8em;
    }

    /*筛选小图标*/
    #implement_list_page .jszx-bar-item-3 div p img {
        width: 0.6em;
        height: 0.5em;
        padding-left: 0.5em;
    }

    #implement_list_page .home_list_tr_first_td span {
        margin-left: 0.5em;
    }
    #implement_list_page .right_img {

        height: 1.5em;
    }
    #implement_list_page .search_div{
        height: 3.2em;
        padding: 0.5em 1em 0em 1em;
        margin: 0em;
        background: #edeef1;
    }
    #implement_list_page #projectname_line{
        display:inline-block;width:5px;height:22px;background:#2b6cd2;vertical-align: middle;
    }
    #implement_list_page #projectname{
        margin-left: 1em;
        display:inline-block;
        vertical-align: middle;
    }
    #implement_list_page .green_font{
        color:#3d8607;
    }
    #implement_list_page .common_font{
        color:#333333;
    }
    #implement_list_page .alldivhight{
        height:24px;
    }

    #implement_list_page .page_header .page_header_title{
        line-height: 2.3em;
        font-size: 1em;
        height:2.3em;
    }
    #implement_list_page .page_header{
        height:2.3em;
    }
    #implement_list_page .page_header img{
        height: 1em;
    }

    #implement_list_page .list_top_down{
    ...
    /* #implement_list_page .handlelist{
      height:3em;
      padding:1em;

    }
    #implement_list_page .lefthandle{
      float:left;
      width:90%;
      border-bottom: 1px solid #dadada;
      height: 3em;
    }
    #implement_list_page .lefthandle >div{
      height: 1.5em;
    }
    #implement_list_page .righthandle{
      line-height: 3em;
      height: 3em;
      float:left;
      width:10%;
