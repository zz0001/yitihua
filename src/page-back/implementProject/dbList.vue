<template>
    ...
    <div data-role="page" id="db_list_page">
        <tk-header>
            实事项目列表
        </tk-header>
        ...
        <div class="page_content" style="">
            ...
            <div id="db_list_wrapper" class="jszx-wrapper">
                <div class="jszx-scroller">
                    <div id="db_list_html" class="html_margin white_background">
...
<script>
    export default {
        ...
        mounted() {
            ...
            /*上报*/
            $("#db_list_page .right_img").listen("quickClick", function (e) {

            });
            /*返回*/
            $("#db_list_page .left_img").listen("quickClick", function (e) {
            ...
            //筛选
            $("#db_list_page .right_btn").listen("quickClick", function (e) {
             ...
             * 初始化
             */
            function initDbWelcome() {
                $("#db_list_wrapper")[0].style.height = window.localInnerHeight - $("#db_list_page .page_header").height() - $("#db_list_page .jszx-bar-icon-up").height() + "px";
                initScroll({
                    "wrapper": "db_list_wrapper",
                    "dir": "y",
                    "bounce": false,
                    "scrollBar": true,
                    "pullUpLoading": function (reset) { //上拉
                        if (window.searchPage > window.arrayDataLength) {
                            ...
                            findUndoSuperviseAndIssue(reset, window.searchPage++);
                        }
                    },
                    "pullDownLoading": function (reset) { //下拉
                        window.searchPage = 2;
                        findUndoSuperviseAndIssue(reset, 1);

                    }
                });
                scrollToTop("db_list_wrapper");

                //开始时间

                //结束时间
                findUndoSuperviseAndIssue(false, 1);
            ...
            //已竣工
            function findUndoSuperviseAndIssue(reset, pageNum) {
                if (!reset) {
                    //showLoading("正在搜索...");
                }
                ...
                var type = window.SuperviseAndIssueType;
                var obj = "";
                ...
                doPost("findUndoSuperviseAndIssue.action", obj, function (data) {
                    if (!reset) {
                        hideLoading();
                    }
                    if (data.total > 0) {
                        ...
                        if (!isNull(array) && array.length > 0) {
                            for (var i = 0; i < array.length; i++) {
                                ...
                                if (array[i].totalInvestment == "" || !array[i].totalInvestment) {
                                    totalinvestment = "0";
                                }
                                if (array[i].investmentamount == "" || !array[i].investmentamount) {
                                    investmentamount = "0";
                                }
                                if (array[i].yearcompleteamount == "" || !array[i].yearcompleteamount) {
                                    yearcompleteamount = "0";
                                }
                                if (array[i].yearcompleterate == "" || !array[i].yearcompleterate) {
                                    yearcompleterate = "0";
                                }
                                var html = '';
                                ...
                                var superviseno = array[i].superviseno;
                                if (!array[i].superviseno) {
                                    superviseno = "无";
                                }
                                var issueno = array[i].issueno;
                                if (!array[i].issueno) {
                                    issueno = "无";
                                }

                                html += '       <td colspan="3" style="border-top:1px solid #dadada;font-size:14px;text-align:left;padding: 0.5em 0;padding-bottom: 0;"><div style="display: inline-block;width:32%;text-align:center;"><div style="height:24px;">检查督办</div><div class="oranger_font" style="height:24px;">' + superviseno + '</div></div> ';
                                html += '<div style="background:#dadada;width: 1px;height:2em;display: inline-block;top: 50%; margin-top: -0.5em;"></div>'
                                html += '<div style="display: inline-block;width:32%;text-align:center;"><div style="height:24px;">存在问题</div><div class="green_font" style="height:24px;color:#3d8607;">' + issueno + '</div></div>';
                                ...
                                var $html = createListRow(html).appendTo("#db_list_html");
                                array[i].id = array[i].rowId;
...
<style scoped>
    #db_list_page .page_header_title .jszx-bar-icon-up {
        background: #fafafa;
    }

    #db_list_page .home_list_tr_first_td {
        padding: 0.7em;
        padding-left: 0em;
    }

    #db_list_page .right_btn {
        width: 3.5em;
        height: 1.5em;
        color: #ffffff;
    }

    #db_list_page .right_btn .btn_div {
        color: #ffffff;
    ...
    #db_list_page .jszx-bar-item-3 > div {
        width: 33.3%;
    }

    #db_list_page .tab_oranger_font p,
    .tab_bloack_font p, #db_list_page .jszx-bar-item-3 div p {
        height: 2.8em;
        line-height: 2.8em;
    }

    /*筛选小图标*/
    #db_list_page .jszx-bar-item-3 div p img {
        width: 0.6em;
        height: 0.5em;
        padding-left: 0.5em;
    }

    #db_list_page .home_list_tr_first_td span {
        margin-left: 0.5em;
    }

    #db_list_page .right_img {

        height: 1.5em;
    }

    #db_list_page .search_div {
        height: 3.2em;
    ...
    #db_list_page #projectname {
        margin-left: 1em;
        display: inline-block;
        vertical-align: middle;
    }

    #db_list_page .green_font {
        color: #3d8607;
    }

    #db_list_page .common_font {
        color: #333333;
    }

    #db_list_page .alldivhight {
        height: 24px;
    }

    #db_list_page .page_header .page_header_title {
        line-height: 2.3em;
        font-size: 1em;
        height: 2.3em;
    }

    #db_list_page .page_header {
        height: 2.3em;
    }

    #db_list_page .page_header img {
        height: 1em;
    }

    #db_list_page .list_top_down {
