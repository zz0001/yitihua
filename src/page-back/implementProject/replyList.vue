<template>
    <div data-role="page" id="reply_page">
        ...
        <tk-header>
            督察督办
            <span
                    @click='$router.push("implementProjectEdit.vue")'
                    slot="right">新增</span>
        </tk-header>

        <div class="page_content">
            <div id="reply_list_wrapper" class="jszx-wrapper" style="background: #fff;">
                <div class="jszx-scroller">
                    <div class="html_margin white_background" id="deply_html">
        ...
        <div class="KpiTcbackG" id="replyblackBg" style="display:none;background:#000;opacity: 0.7;">
        </div>

        <div class="KpiFooterContent2" id="replysupervisDiv" style="display:none;">
            <p class="KpiNameTitle"></p>
            <table class="inp_table" cellpadding="0" cellspacing="0">
                <tr>
                    <td style="padding: 1em;">督办回复</td>
                    <td style="text-align:right;"><span style="color:#3977E6;"
                                                        id="configbackhandlername_editreply">发表</span></td>
                </tr>
                <tr>
                    <td colspan="2" style="padding: 0;">
                           <textarea
                                   onfocus="newCheckWord('reply_edit_problem',500,'reply_case_words')"
                                   onKeyUp="javascript:newCheckWord('reply_edit_problem',500,'reply_case_words');"
                                   onMouseDown="javascript:newCheckWord('reply_edit_problem',500,'reply_case_words');"
                                   placeholder="请输入你的回复内容" id="reply_edit_problem"
                                   style="text-align:left;min-height: 5em;padding:0em 0.5em 0em 1em;"></textarea>

                        <input class="small gray_dark_font" style="color:#999999;width: 58%;" readonly="true"
                               id="reply_case_words"/>
                    </td>
                </tr>
            </table>
...
<script>
    export default {
        ...
        mounted() {
            ...
            function recordAudio() {
                ...
                //定时器
                try {
                    clearInterval($('#reply_page').data('rememberTimer'));
                } catch (e) {
                }
                var rememberCount = 0
                ...
                //定时器end
                $('#reply_page').data('mediaRec', mediaRec);
                $('#reply_page').data('rememberTimer', rememberTimer);
            ...
            function recordFinish() {
                var mediaRec = $('#reply_page').data('mediaRec');
                mediaRec.stopRecord();
                mediaRec.release();
                $(".recordStart").hide();
                $(".recordFinish").show();
            ...
            $("#reply_list_wrapper")[0].style.height = window.localInnerHeight - $("#reply_page .page_header").height() + "px";
            window.searchPage = 2;
            initScroll({
                "wrapper": "reply_list_wrapper",
                "dir": "y",
                "bounce": false,
                "scrollBar": true,
                "pullUpLoading": function (reset) { //上拉
                    if (window.searchPage > window.arrayDataLength) {
                        ...
                        replyfindSuperviseList(reset, window.searchPage++);
                    }
                },
                "pullDownLoading": function (reset) { //下拉
                    window.searchPage = 2;

                    replyfindSuperviseList(reset, 1)
                }
            });
            ...
            function initReplyPage() {
                ...
                //获得详情
                replyfindSuperviseList(false, 1);
                //
                $('#reply_page').data('mediaRec', null);
            ...
            function initEvent() {
                ...
                $(".recordFinish").hide();
                $('#reply_page').data('mediaRec', null);
                $('#reply_page').data('rememberTimer', null);
                //语音录入初始化end

                //回复
                $("#deply_html").on("click", ".reply", function (e) {

                    window.replyId = $(e.currentTarget).attr("replyId");
                    $("#reply_edit_problem").val("");
                    $("#reply_case_words").val("0/500");
                    $("#replyblackBg").show();
                    $("#replysupervisDiv").show();
                    // $("#replysupervisDiv").addClass("kdts-botwrap-open");
                });
                //关闭
                $("#deply_html").on("click", ".close", function (e) {
                    var obj = "";
                    window.replyId = $(e.currentTarget).attr("replyId");
                    obj += toJson("superviseid", window.replyId) + "&";

                    obj += toJson("isClosed", "1");

                    doPost("findSuperviseReply.action", obj, function (data) {
                ...
                $("#reply_page #configbackhandlername_editreply").listen("quickClick", function (e) {
                    findSuperviseReply();
                });
                //选择人员
                $("#reply_page #peopleselect_tr").listen("quickClick", function (e) {
                    selsectPeople();
                });


                $("#reply_page #replyblackBg").listen("quickClick", function (e) {

                    $("#replyblackBg").hide();

                    $("#replysupervisDiv").hide();
                });
                ...
                //后退
                $("#reply_page .back-button").listen("quickClick", function (e) {
                ...
                //处置部门
                $("#reply_page #handlername_edit").listen("quickClick", function (e) {
                    app_handlername();
                });
                chooseDate("appEnfor_end_time", "datetime");
                //商品图片
                $("#reply_page #appEnforchoice").click(function (e) {
                ...
                $(".deptrecordPlay").click(function () {
                    if (!window.flag) {
                        ...
                        $('#reply_page').data('mediaRec', mediaRecObj);
                    }
                    mediaRecObj.play();
                    /*   if(status==0){
                          setTimeout(function(){
            ...
            function readerMedia() {
                if (!$('#reply_page').data('mediaRec')) {//无语音文件
                    saveImplementEdit();
                    return;
                }
   ...
   */
            ...
            function initImplementEditPageData() {
                //任务详情
                $("#reply_page").data("taskdetail", null);
                //部门列表
                $("#reply_page").data("centerTaskDept", null);
                //选中部门
                $("#reply_page").data("taskDept", null);
                //任务编号
                $("#reply_page").data("taskno", "");
                //选中部门数组
                $("#reply_page").data("taskDeptArray", []);
            }
            ...
            function replyfindSuperviseList(reset, pageNum) {
                if (!reset) {
                    //showLoading("正在搜索...");
                }
                ...
                doPost("findSuperviseList.action", obj, function (data) {
                    if (!reset) {
                        hideLoading();
                    }
                    if (data.total > 0) {
                        ...
                        if (!isNull(array) && array.length > 0) {
                            ...
                            for (var i = 0; i < array.length; i++) {
                                ...
                                html += '<div style="background: #fff;"><div style="padding: 0.5em;">' + array[i].suggestion + '</div>';
                                ...
                                if (array[i].isClosed == "1") {
                                    ...
                                    html += '<span replyId="' + array[i].id + '" class="reply" style="border:1px solid #ccc;padding: 5px;border-radius: 5px;margin-right: 20px;">回复</span>';
                                    html += '<span replyId="' + array[i].id + '" class="close" style="border:1px solid #ccc;padding: 5px;border-radius: 5px;margin-right: 20px;">关闭</span>';
                                }
                                ...
                                if (array[i].reply.length > 0) {
                                    ...
                                    for (var j = 0; j < array[i].reply.length; j++) {
                                        html += '<table class="inp_table" cellpadding="0" cellspacing="0" style="margin-left: 5px;background: #ededed;width:97%;border-bottom: 1px solid #ccc;">';
                                        html += ' <tr style="background: #ededed;"><td style="padding: 1em;color:#3977E6">回复人：' + array[i].reply[j].replyName + '</td><td style="text-align:center;">' + array[i].reply[j].createTime + '</td></tr>';
                                        html += '<tr style="background: #ededed;"><td colspan="2" style="padding: 1em;color:#555">' + array[i].reply[j].replyContent + '</td></tr></table>';
                                    }
                                    html += '</div>'
                                }
                                html += '</div>';
            ...
            /*加载页面数据*/
            function findSuperviseReply(id) {
                var content = $("#reply_edit_problem").val();
                var obj = "";
                obj += toJson("superviseid", window.replyId) + "&";
                ...
                doPost("findSuperviseReply.action", obj, function (data) {
                    ...
                    if (data.code == "0") {
                        $("#replyblackBg").hide();
                        /*   $("#replysupervisDiv").removeClass("kdts-botwrap-open"); */
                        $("#replysupervisDiv").hide();
                        toast(data.message);
                        replyfindSuperviseList(false, 1);
                    }
                    ...
                    success: function (data) {
                        ...
                        if (!isNull(data) && data.length > 0) {
                            ...
                            for (var i = 1; i < array.length; i++) {
                                ...
                                $('#reply_page').find('.checkboxList').append(info1);

                            }
                            //确定按钮
                            $('#reply_page').find('.KpiFormBut').listen("quickClick", function (e) {
                                var str = '';
                                var codestr = "";
                                if (window.doUsersArr.length > 0) {
                                    for (var i = 0; i < window.doUsersArr.length; i++) {
                                ...
                                $('#reply_page').find('#peopleKpiTcbackG').hide();
                                $("#peopleselect_edit").val(str);
                                window.localStorage.currhandlernameCodes = codestr;

                            });
                            $('#reply_page').find('#peopleKpiTcbackG').show();
...
<style scoped>
    #reply_page .page_header .right_img {
        width: 2em;
        top: 0.2em;
    }

    /*img start*/
    #reply_page .commodityAddImgs {
        overflow: hidden;
        background: #f3f3f3;
    }
    ...
    /*img end*/
    #reply_page .div_input_value_img {
        width: 1.5em;
        height: 1.5em;
        padding-right: 0.5em;
    }
    ...
    #reply_page .page_header .page_header_title {
        line-height: 2.3em;
        font-size: 1em;
        height: 2.3em;
    }

    #reply_page .page_header {
        height: 2.3em;
    }

    #reply_page .page_header img {
        height: 1em;
    }
</style>
