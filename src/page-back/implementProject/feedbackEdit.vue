<template>
    <div data-role="page" id="feed_edit_page">
        ...
        <div class="page_content" >
            <div id="feed_edit_wrapper" class="jszx-wrapper">
                        ...
                        <table class="inp_table" cellpadding="0" cellspacing="0">
                            ...
                            <tr>
                                ...
                                <td colspan="2" class="gray_line_bottom">
                                    <input class="small gray_dark_font" style="color:#999999;text-overflow: ellipsis;" readonly
                                           id="projectname_edit_feed"/>
                                </td>
                            </tr>
                            <tr>
                                <td style="padding-top:0.5em;border-bottom: 1px solid #f3f3f3">问题选择</td>
                                <td colspan="2" class="gray_line_bottom">
                                    <input class="small gray_dark_font" style="color:#999999;"
                                           id="question_select_feed"/>
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td colspan="2" class="gray_line_bottom">
                                    <input class="small gray_dark_font" style="color:#999999;" readonly="true"
                                           id="supervisesuggestion_feed"/>
                                </td>
                            </tr>
                            <tr >
                                <td colspan="3" class="gray_line_bottom">
                            <textarea
                                    onfocus="newCheckWord('supervisesuggestion_feed_problem',500,'supervisesuggestion_feed')"
                                    onKeyUp="javascript:newCheckWord('supervisesuggestion_feed_problem',500,'supervisesuggestion_feed');"
                                    onMouseDown="javascript:newCheckWord('supervisesuggestion_feed_problem',500,'supervisesuggestion_feed');"
                                    placeholder="请增加你的描述" id="supervisesuggestion_feed_problem"
                                    style="text-align:left;min-height: 5em;padding:0em 0.5em 0em 1em;"></textarea>

                                </td>
                            </tr>
                            <tr id="leadfeed_tr"><td colspan="3" style="padding: 1em;">督办人  <span ></span></td></tr>
                            <tr id="leadfeedtime_tr"><td colspan="3" style="padding: 1em;">督办时间  <span ></span></td></tr>
        ...
        <div class="page_footer">
            <div class="footer_button_div">
                <div id="save_feed_edit" class="alert_bottom_three orange_background">确 认</div>
            </div>
        </div>
    ...
    export default {
        ...
        mounted(){
            ...
            function recordAudio() {
                ...
                //定时器
                try{clearInterval( $('#feed_edit_page').data('rememberTimer'));}catch(e){}
                var rememberCount = 0
                ...
                //定时器end
                $('#feed_edit_page').data('mediaRec',mediaRec);
                $('#feed_edit_page').data('rememberTimer',rememberTimer);
            ...
            function recordFinish() {
                var mediaRec = $('#feed_edit_page').data('mediaRec');
                mediaRec.stopRecord();
                mediaRec.release();
                $(".recordStart").hide();
                $(".recordFinish").show();
                clearInterval( $('#feed_edit_page').data('rememberTimer'));
            }
                //设置标题栏，内容栏，底部内容高度
                ...
                $("#feed_edit_wrapper")[0].style.height = window.localInnerHeight - $("#feed_edit_page .page_header").height() + "px";

                initScroll({
                    "wrapper": "feed_edit_wrapper",
                    "dir": "y",
                    "bounce": false,
                    "scrollBar": true
                });
                ...
                function initLeaderHandlePage(){
                    ...
                    //getCenterTaskdetail();
                    //
                    $('#feed_edit_page').data('mediaRec',null);
                ...
                function initEvent(){
                    ...
                    $(".recordFinish").hide();
                    $('#feed_edit_page').data('mediaRec',null);
                    $('#feed_edit_page').data('rememberTimer',null);
                    //语音录入初始化end
                    ...
                    $("#leadfeed_tr").html(htmls);
                    $("#leadfeedtime_tr").html(htmls2);
                    $("#projectname_edit_feed").val(window.projectname);
                    ...
                    $("#save_feed_edit").listen("quickClick", function (e) {

                        if (isNull($("#projectname_edit_feed").val())) {
                            toast("请输入项目名称");
                        }else if (isNull($("#handlername_edit_feed").val())) {//window.doUsersArr
                            toast("请输入督察人姓名");
                        }else if (isNull($("#supervisesuggestion_feed_problem").val())) {//window.doUsersArr
                            toast("请输入问题反馈");
                        } else {
                    ...
                    $("#feed_edit_page").on("click", ".KpiTcbackG", function (e) {


                    });
                    //后退
                    $("#feed_edit_page .back-button").listen("quickClick", function (e) {
                        that.$router.back();
                    });
                    //处置人员
                    $("#feed_edit_page #handlername_edit_feed").listen("quickClick", function (e) {
                        app_handlername();
                    });
                    //处置问题
                    $("#feed_edit_page #question_select_feed").listen("quickClick", function (e) {
                        app_findQuestionType();
                    });
                    chooseDate("appEnfor_end_time", "datetime");
                    //商品图片
                    $("#feed_edit_page #appEnforchoice").click(function (e) {
                    ...
                    $(".deptrecordPlay").click(function(){
                        if(!window.flag){
                            ...
                            $('#feed_edit_page').data('mediaRec',mediaRecObj);
                        }
                        mediaRecObj.play();
                        /*   if(status==0){
                              setTimeout(function(){
                ...
                function readerMedia(){
                    if(!$('#feed_edit_page').data('mediaRec')){//无语音文件
                        saveImplementEdit();
                        return;
                    }
                ...
                function initImplementEditPageDom(){
                    $("#projectname_edit_feed").val("");
                    $("#handlername_edit_feed").val("");
                    $("#question_select_feed").val("");
                    $("#supervisesuggestion_feed_problem").val("");
                ...
                function initImplementEditPageData(){
                    //任务详情
                    $("#feed_edit_page").data("taskdetail",null);
                    //部门列表
                    $("#feed_edit_page").data("centerTaskDept",null);
                    //选中部门
                    $("#feed_edit_page").data("taskDept",null);
                    //任务编号
                    $("#feed_edit_page").data("taskno","");
                    //选中部门数组
                    $("#feed_edit_page").data("taskDeptArray",[]);
                ...
                /*     function getCenterTaskdetail(){
                        if(window.repairListParam_0329_yfy){
                            ...
                            doPost("centerTaskdetail.action", obj, function (data) {
                                $("#feed_edit_page").data("taskdetail",data);
                                //加载页面数据
                                loadTaskdetail(data);
                            }, "");
                        }
...
//领导直派，图片上传
                ...
                function saveImplementEdit(src){
                    ...
                    var handlername=$("#handlername_edit_feed").val();
                    var questiontype=$("#question_select_feed").val();

                    var supervisesuggestion_feed_problem=$("#supervisesuggestion_feed_problem").val();
                    //接口入参
                    var obj = "";
                    obj += toJson("id", id) + "&";
                    ...
                    obj += toJson("questiontype", questiontype) + "&";
                    obj += toJson("supervisesuggestion", supervisesuggestion_feed_problem);
                    //集中整治详情接口
                    doPost("issueFeedback.action", obj, function (data) {
                ...
                function loadTaskAppEnfor(list){
                    ...
                    for (var i=0;i<list.length;i++) {
                        ...
                        $listStr.listen("quickClick", function (e) {
                            ...
                            //选中部门数据
                            // $("#feed_edit_page").data("taskDept",data);
                            $("#feed_edit_page").data("taskDeptArray",arraylist);
                        });
                    };
                    //默认选中第一个
                    //$("#feed_edit_page").data("taskDept",list[0]);
                    ...
                    success: function (data) {
                        ...
                        if (!isNull(data) && data.length > 0) {
                            ...
                            for (var i = 1; i < array.length; i++) {
                                ...
                                $('#feed_edit_page').find('.checkboxList').append(info1);

                            }
                            //确定按钮
                            $('#feed_edit_page').find('.KpiFormBut').listen("quickClick", function (e){
                                var str = '';
                                var codestr = "";
                                /*                                     if(window.doUsersArr.length>0){
                                                                        for(var i=0 ; i < window.doUsersArr.length; i++){
                                ...
                                $('#feed_edit_page').find('.KpiTcbackG').hide();
                                $("#handlername_edit_feed").val(str);
                                window.localStorage.currhandlernameCodes=codestr;

                            });
                            $('#feed_edit_page').find('.KpiTcbackG').show();
            ...
            //部门名称
            function app_findQuestionType(type, source) {
                var obj = "";
                $('.KpiTcbackG2').remove();
                var info = '<div class="KpiTcbackG2" style="display:none;">'+
                ...
                doPost("findQuestionType.action", obj, function (data) {
                    if (!isNull(data) && data.length > 0) {
                        var array = data;
                        for (var i = 0; i < array.length; i++) {
                            ...
                            info1 = $('<div style="padding:1.5% 0;display:inline-block;width:50%;vertical-align:middle;">'+
                                '<input type="radio" value="" class="disI vm">'+
                                '<span class="disI vm f14" style="margin-left:3%;">'+array[i].item_text+'</span>'+
                                ' </div>');
                            info1.data('lidata',array[i]);
                            info1.listen("quickClick", function (e) {
                                $(".KpiTcbackG2").find('input').attr('checked', false);
                                var isCheck  = $(e.currentTarget).find('input').is(":checked");
                                ...
                                if(!isCheck){//false一开始未选中   true，已被选中
                                    $(e.currentTarget).find('input').attr('checked', true);
                                    if(window.doUsersArr2.length>0){
                                        var flag = null;
                                        for(var i=0 ; i < window.doUsersArr2.length; i++){
                                            if(data.item_value == window.doUsersArr2[i].item_value){
                                                flag = true;
                                                break;
                                            }
                                        }
                                        if(!flag){
                                            window.doUsersArr2=data;
                                        }
                                    }else{
                                        window.doUsersArr2=data;
                                    }
                                    ...
                                    if(index!=-1){
                                        window.doUsersArr2.splice(index,1);
                                    }
                                }

                                //console.error("data:"+JSON.stringify(data))
                                //console.error("window.doUsersArr2:"+JSON.stringify(window.doUsersArr2))
                        ...
                        $('#feed_edit_page').find('.KpiFormBut').listen("quickClick", function (e){
                            ...
                            str=window.doUsersArr2.item_text;
                            codestr=window.doUsersArr2.item_value;
                            $('#feed_edit_page').find('.KpiTcbackG2').hide();
                            $("#question_select_feed").val(str);
                            window.localStorage.questiontype=codestr;

                        });
                        $('#feed_edit_page').find('.KpiTcbackG2').show();
...
<style scoped>
    #feed_edit_page .page_header .right_img {
        width: 2em;
        top: 0.2em;
    }

    /*img start*/
    #feed_edit_page .commodityAddImgs {
        overflow: hidden;
        background: #f3f3f3;
    }

    #feed_edit_page .imgdiv {
        padding: 0.7em;
        padding-top: 0em;
        position: relative;
        width: 6em;
    ...
    #feed_edit_page #appEnfor_edit_img {
        height: 7.5em;
        overflow: hidden;
    ...
    /*img end*/
    #feed_edit_page .div_input_value_img {
        width: 1.5em;
        height: 1.5em;
        padding-right: 0.5em;
    }
    ...
    .KpiTcbackG .Tipscontent,.KpiTcbackG2 .Tipscontent2{
        width:81%;
        margin:40% auto 0;
        background: #fff;
        border-radius: 6px;
    ...
    #feed_edit_page .page_header .page_header_title{
        line-height: 2.3em;
        font-size: 1em;
        height:2.3em;
    }
    #feed_edit_page .page_header{
        height:2.3em;
    }
    #feed_edit_page .page_header img{
        height: 1em;
    }
</style>
