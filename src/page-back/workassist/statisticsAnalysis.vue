<template>
    <div data-role="page" id="analysis_welcome_page">
        ...
        <div class="page_condition">
            ...
            <div class="queryClick" id="queryClick"><span>点击查看</span></div>
        </div>
        <div id="analysis_welcome_wrapper" class="jszx-wrapper">
...
<script type="text/ecmascript-6">
    export default {
        ...
        mounted() {
            var that = this;
            // $("#analysis_welcome_page").live("pageshow", function () {
                // 先获取部门
                get_department();
                // 在获取职能事项
                get_zhineng();
                ...
                // 返回
                $("#analysis_welcome_page .left_img").listen("quickClick", function (e) {
                ...
                // 改变职能大类
                $("#analysis_welcome_page #zhineng").change(function (e) {
                    get_bigClass($("#analysis_welcome_page #zhineng").val());
                })
                // 点击查询
                $("#queryClick").listen("quickClick", function () {
                    app_statisticsList($("#analysis_welcome_page #zhineng").val(), $("#analysis_welcome_page #bigClass").val(), $("#analysis_welcome_page #bumen").val(), $("#analysis_welcome_page #time").val());
                })
            // });

            //初始化
            function statisticsInit() {
                $("#analysis_welcome_wrapper")[0].style.height = window.localInnerHeight - $("#analysis_welcome_page .page_header").height() - $("#analysis_welcome_page .page_condition").height() + "px";
                initScroll({
                    "wrapper": "analysis_welcome_wrapper",
                    "dir": "y",
                    "bounce": false,
                    "scrollBar": true,
                });
                scrollToTop("analysis_welcome_wrapper");
                app_statisticsList("", "", "", 1);
            }
            ...
            function get_department() {
                doPost("deptList.action", "", function (data) {
                    ...
                    $("#bumen").html(html);
                    $("#bumen").prev().children(".ui-btn-text").find("span").html("默认(全部)");
                }, "")
            }

            // 获取职能
            function get_zhineng() {
                doPost("findMultiCodeList.action", "", function (data) {
                    ...
                    $("#zhineng").html(html);
                    $("#zhineng").prev().children(".ui-btn-text").find("span").html("默认(全部)");
                }, "")
            }

            // 获取大类
            function get_bigClass(zhineng) {
                if (zhineng == "") {
                    var html = '<option selected="selected" value="">默认(全部)</option>'
                    $("#bigClass").html(html);
                    $("#bigClass").prev().children(".ui-btn-text").find("span").html("默认(全部)");
                } else {
                    var obj = "";
                    obj += toJson("selDept", zhineng);
            ...
            // 请求数据
            function app_statisticsList(zhineng, bigClass, bumen, time) {
                var obj = "";
                obj += toJson("component", zhineng) + "&";
                obj += toJson("bigclass", bigClass) + "&";
                obj += toJson("gridCode", bumen) + "&";
                obj += toJson("duration", time);
                doPost("functionStatistics.action", obj, function (data) {
                    var html = "";
                    ...
                    html += '<thead><tr><th>职能事项</th><th>大类</th><th>工单数</th><th>按时办结率</th><th>超期未处置数</th></tr></thead>';
                    if (data.datas.length == 0) {
                    }
                    for (var i = 0; i < data.datas.length; i++) {
                        html += '<tbody><tr><td>' + data.datas[i].componmentName + '</td><td>' + data.datas[i].largeClassName + '</td><td>' + data.datas[i].workOrder + '</td><td>' + data.datas[i].complete + '</td><td>' + data.datas[i].overUndispose + '</td></tr></tbody>';
                    }
                    html += '</table></div>';
                    $("#statistics_html").html(html);
                }, "")
            }
        },
        methods: {}
